...
export default function AdminSidePanel() {
    ...
    return (
        <div className="group text-white bg-(--brand-primary) fixed flex flex-col transition-all duration-300 h-full overflow-hidden z-40">
            ...
            <div className="flex flex-col justify-between h-full bg-(--brand-primary)">
                {/* MENU */}
                <nav className="flex-1 flex flex-col">
                    {menuItems.map((item) => (
                        <NavLink
                            key={item.label}
                            to={item.to}
                            className={({ isActive }) => `flex items-center gap-3 p-4 font-bold hover:text-black hover:bg-white transition-all duration-300
                ...
                <button
                    onClick={logout}
                    className="flex items-center gap-3 p-4 w-full h-16 font-bold text-white hover:bg-red-500 transition-all duration-300 rounded cursor-pointer"
                >
                    <LogOut size={20} />
                    <span className="whitespace-nowrap transition-all duration-300">
                        Logout
